...
//    _phoneNumber.layer.cornerRadius  = _phoneNumber.frame.size.height/16;
    ...
    _mynewWord = [[UITextField alloc]init];
    _mynewWord.frame = CGRectMake(0, 210*hb, SCREEN_WIDTH, 100*hb);
    ...
    _mynewWord.backgroundColor = RGBA(38, 40, 52, 1);
    ...
    [_mynewWord addSubview:_mynewWordleftview];
    ...
    _mynewWord.leftView = _mynewWordleftview;
    _mynewWord.leftViewMode=UITextFieldViewModeAlways;
    
    
    _mynewWord.placeholder =@"6-12位数字和字母组合";
    [_mynewWord setValue:RGBA(150, 160, 190, 1) forKeyPath:@"_placeholderLabel.textColor"];
    [_mynewWord setValue:[UIFont systemFontOfSize:13] forKeyPath:@"_placeholderLabel.font"];

    //确认登录密码
    
    _comformWord = [[UITextField alloc]init];
    _comformWord.frame = CGRectMake(0, 210*hb +100*hb + 1, SCREEN_WIDTH, 100*hb);
    [self.view addSubview:_comformWord];
    _comformWord.delegate = self;
    _comformWord.tag = 104;
    _comformWord.returnKeyType = UIReturnKeyDone;
    //    _phoneNumber.layer.masksToBounds = YES;
    //    _phoneNumber.layer.cornerRadius  = _phoneNumber.frame.size.height/16;
    
    _comformWord.backgroundColor = RGBA(38, 40, 52, 1);
    
    UIView * _comformWordleftview = [[UIView alloc]init];
    _comformWordleftview.frame = CGRectMake(0, 0, 230*wb, 100*hb);
    _comformWordleftview.backgroundColor =RGBA(38, 40, 52, 1);
    [_comformWord addSubview:_comformWordleftview];
    
    UILabel * _comformWordlabel = [[UILabel alloc]init];
    _comformWordlabel.frame =CGRectMake(30*wb, 33*hb, 200*wb, 30*hb);
    _comformWordlabel.textColor = [UIColor whiteColor];
    _comformWordlabel.text =@"确认登录密码";
    if (SCREEN_WIDTH == 320) {
        _comformWordlabel.font = [UIFont systemFontOfSize:13];
    }else
    {
       _comformWordlabel.font = [UIFont systemFontOfSize:15];
    }
    _comformWordlabel.textAlignment = NSTextAlignmentLeft;
    [_comformWordleftview addSubview:_comformWordlabel];
    _comformWord.leftView = _comformWordleftview;
    _comformWord.leftViewMode=UITextFieldViewModeAlways;
    
    
    _comformWord.placeholder =@"请输入确认登录密码";
    [_comformWord setValue:RGBA(150, 160, 190, 1) forKeyPath:@"_placeholderLabel.textColor"];
    [_comformWord setValue:[UIFont systemFontOfSize:13] forKeyPath:@"_placeholderLabel.font"];

    
    //确定
    _qdButton = [[UIButton alloc]init];
    _qdButton.frame = CGRectMake((SCREEN_WIDTH-580*wb)/2, 545*hb, 580*wb, 80*hb);
    [_qdButton setBackgroundColor:APP_BLUE];
    ...
    if (SCREEN_WIDTH == 320) {
        _qdButton.titleLabel.font = [UIFont systemFontOfSize: 16.0];
    }else
    {
        _qdButton.titleLabel.font = [UIFont systemFontOfSize: 18.0];
    }
    [_qdButton addTarget:self action:@selector(qdbuttonClick) forControlEvents:UIControlEventTouchUpInside];
    _qdButton.contentHorizontalAlignment = UIControlContentHorizontalAlignmentCenter;
